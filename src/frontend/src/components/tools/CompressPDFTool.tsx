...
export default function CompressPDFTool() {
  ...
  const handleDownload = () => {
    if (!result) return;
    const blob = new Blob([result.compressedBytes.buffer as ArrayBuffer], {
  ...
  const couldNotReduce =
    result !== null && result.compressedSize >= result.originalSize;
        ...
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          ...
          {couldNotReduce && (
            <div className="flex items-center gap-2 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-600 dark:text-yellow-400 text-sm">
              ...
              The file could not be reduced further. The PDF may already be
              optimized.
            </div>
          )}
